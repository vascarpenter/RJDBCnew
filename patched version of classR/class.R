...
JDBC <- function(driverClass='', classPath='', identifier.quote=NA) {
  ...
  .jaddClassPath(system.file("java", "RJDBCnew.jar", package="RJDBC"))
  ## added oracle JDBC specific jar
  .jaddClassPath(system.file("java", "ojdbc8.jar", package="RJDBC"))
  .jaddClassPath(system.file("java", "oraclepki.jar", package="RJDBC"))
  .jaddClassPath(system.file("java", "osdt_core.jar", package="RJDBC"))
  .jaddClassPath(system.file("java", "osdt_cert.jar", package="RJDBC"))
  .jaddClassPath(system.file("java", "ucp.jar", package="RJDBC"))
...
setMethod("dbConnect", "JDBCDriver", def=function(drv, url, user='', password='', ...) {
  if (grepl("oracle:thin:", url)) {  # oracle thin JDBC string included
  	# print(url) # for debug
  	v<-.jnew("info/urbanek/Rpackage/RJDBC/JDBCGetConnectionOracle")
    jc <- .jcall(v, returnSig="Ljava/sql/Connection;", method="getConnection", as.character(url)[1], as.character(user)[1], as.character(password)[1], check=FALSE)
  } else {
